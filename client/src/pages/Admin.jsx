...
function Admin({ walletAddress, connected }) {
...
const fetchElections = async () => {
  ...
  const handleEndElection = async (electionId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh elections list
        const refreshResponse = await fetch(`${config.API_URL}/elections`);
        const refreshData = await refreshResponse.json();
        
        if (refreshData.success) {
          setElections(refreshData.elections);
        }
      } else {
  ...
  return (
    <div>
      ...
      // In Admin.jsx, add this somewhere in the render
        <div className="mt-4">
